...
def P1_Crawling_Basic(participant):
	...
	if num==len(ad_lists):
		print("The participant has same %d number of audiobooks"%num)
		print(ad_lists, sep='n')
		return
			...
			# 각 오디오북 상세 페이지 접속 
				...
				for elem in elems:
					try:
						...
						info_playtime=elem.find_element_by_css_selector(css_info_playtime).text
						info_img_url=elem.find_element_by_css_selector(css_info_url).get_attribute("src")
						dict_elem[info_title]={'author':info_author, 'narrator':' ', 'playtime':info_playtime, 'img_url':info_img_url}
						#print(dict_elem)
