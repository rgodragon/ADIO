...
def thread_data():
    try:
        ...
        #cur_data, finFlag, onNum, shelfNum, index=data.readCur(p.name)
        cur_data, finFlag, onNum = data.readCur(p.name)
        ...
        #print('index = ', index, ' onNum = ', onNum, 'finFlag = ', finFlag)
            ...
            if b.read('sh') == True:
                ...
                if nowFlag==0 :
                    ...
                    if finFlag==0:
                        ...
                        epaper.changePIL(p.name, 'empty')
                        time.sleep(0.5)
                        log.Log(p.name, 'Ongoing_Now', 'empty', '0')
                        time.sleep(0.5)
                        #cur_progress=motor.move(cur_progress, prg_data[cur_data['most_recent_adb']]['overall_prg'])
